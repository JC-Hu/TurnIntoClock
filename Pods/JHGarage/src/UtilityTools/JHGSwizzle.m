...
@implementation NSObject (JHGSwizzle)
...
+ (BOOL)jhg_swizzleClassMethod:(SEL)origSel_ withClassMethod:(SEL)altSel_ error:(NSError**)error_ {
	return [GetClass((id)self) jhg_swizzleMethod:origSel_ withMethod:altSel_ error:error_];
}

+ (NSInvocation*)jhg_swizzleMethod:(SEL)origSel withBlock:(id)block error:(NSError**)error {
    IMP blockIMP = imp_implementationWithBlock(block);
    NSString *blockSelectorString = [NSString stringWithFormat:@"_jhg_block_%@_%p", NSStringFromSelector(origSel), block];
    ...
    [self jhg_swizzleMethod:origSel withMethod:blockSel error:nil];

    return origInvocation;
}

+ (NSInvocation*)jhg_swizzleClassMethod:(SEL)origSel withBlock:(id)block error:(NSError**)error {
    NSInvocation *invocation = [GetClass((id)self) jhg_swizzleMethod:origSel withBlock:block error:error];
