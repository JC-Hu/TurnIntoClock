...
@implementation UIImage (JHGImageUtil)
                     ...
                     quality:(CGInterpolationQuality)quality
{
    ...
    if (((int)(bnds.size.width*2))%2 == 1) {
        bnds.size.width = bnds.size.width - .5;
    }
    if (((int)(bnds.size.height*2))%2 == 1) {
        bnds.size.height = bnds.size.height - .5;
    }
...
//    CGImageRelease(imageRef);
    ...
    [self drawInRect:CGRectMake(bounds.origin.x, bounds.origin.y, self.size.width, self.size.height)];
...
#pragma mark -
- (NSData *)compressToKB:(CGFloat)sizeKB
{
    if (sizeKB <= 0) {
        sizeKB = 200;
    }
    ...
    NSLog(@"%.2fKB", imageData.length/1024.0);
    
//    if (imageData.length/1024.0 > sizeKB) {
//        
//        float radio =  sizeKB/(imageData.length/1024.0);
//        
//        imageData = UIImageJPEGRepresentation(self, radio);
//        
//        NSLog(@"%.2fKB", imageData.length/1024.0);
//    }
    float radio = 1;
    while (imageData.length/1024.0 > sizeKB && radio > 0) {
        
        float radio2 =  sizeKB * 4 / (imageData.length/1024.0);
        
        radio = radio2 < radio ? radio2 : radio - .1;
        
        imageData = UIImageJPEGRepresentation(self, radio);
        
        NSLog(@"radio : %.2f, %.2fKB", radio, imageData.length/1024.0);
    }
...
//- (UIImage *)scaleWithLimitHeightOrWidth:(CGFloat)limit
//{
//    CGSize size = CGSizeMake(1, 1);
//    
//    UIImage *newImage = [self scaleWithMaxSize:limit];
//    
//    return newImage;
//}
